...
export function EnhancedStaffCalendarView({
  ...
  const handleMouseUp = useCallback(async () => {
    if (dragState.isDragging && dragState.draggedJob) {
      if (dragState.isResizing && dragState.resizeDirection && onJobUpdate) {
        // Handle resize operation
        const job = dragState.draggedJob;
        const currentDueDate = job.dueDate ? new Date(job.dueDate) : new Date();

        // Calculate new duration based on resize direction
        // This would need backend support for job duration
        console.log(`Resizing job ${job.id} ${dragState.resizeDirection}`);
        // For now, just show the interaction worked
      } else if (dragState.targetDate && dragState.targetTime) {
        // Handle normal move operation
        const [hours] = dragState.targetTime.split(":");
